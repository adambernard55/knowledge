  ...
  sync-docs:
    ...
    steps:
      ...
      - name: Sync to Knowledge Base
        ...
        run: |
          ...
          if [ "${{ github.event_name }}" == "workflow_dispatch" ]; then
            echo "Manual trigger detected - processing ALL files in Knowledge/AI/"
            files=($(find Knowledge/AI -name "*.md" -type f))
          else
          ...
          for file in "${files[@]}"; do
            ...
            case "$file" in
              *"/ai-foundation-models/"*) TOPIC_ID=1596 ;;
              *"/analytics-data-insights/"*) TOPIC_ID=1597 ;;
              *"/audio-generation/"*) TOPIC_ID=1598 ;;
              *"/coding-development/"*) TOPIC_ID=1599 ;;
              *"/content-creation/"*) TOPIC_ID=1600 ;;
              *"/image-video-generation/"*) TOPIC_ID=1601 ;;
              *"/marketing-automation/"*) TOPIC_ID=1602 ;;
              *"/marketing-automation/seo-optimization/"*) TOPIC_ID=1603 ;;
	          *"/productivity-workflow/"*) TOPIC_ID=1604 ;;
	          *"/research-knowledge-agents/"*) TOPIC_ID=1605 ;;
	          *"/social-media/"*) TOPIC_ID=1605 ;;
            esac
            ...
            if grep -q "^---$" "${file}"; then
              ...
              # Extract tags (handles both inline and YAML list format)
              tags=$(echo "$front_matter" | grep -i "^tags:" | sed 's/^tags: *//I' | sed 's/[\[\]]//g' | sed 's/,/ /g')
              
              # If tags is empty, check for YAML list format (tags on separate lines with "  - ")
              if [ -z "$tags" ]; then
                # Extract YAML list items (lines starting with "  - " after "tags:")
                in_tags=false
                while IFS= read -r line; do
                  if [[ "$line" =~ ^tags: ]]; then
                    ...
                    if [[ "$line" =~ ^[[:space:]]*-[[:space:]](.+) ]]; then
                      ...
                      # Hit next field, stop collecting tags
                      break
                    fi
                  fi
                done <<< "$front_matter"
                tags=$(echo "$tags" | xargs)  # Trim whitespace
              fi
      ...
      - name: Summary
        ...
        run: |
          if [ "${{ github.event_name }}" == "workflow_dispatch" ]; then
            echo "AI sync completed (manual trigger - processed all files)"
          else
            echo "AI sync completed (push trigger - processed changed files)"
          fi
