  ...
  sync-docs:
    ...
    steps:
      ...
      - name: Sync to Knowledge Base
        ...
        run: |
          ...
          if [ "${{ github.event_name }}" == "workflow_dispatch" ]; then
            echo "Manual trigger detected - processing ALL files in Knowledge/SEO/"
            files=($(find Knowledge/SEO -name "*.md" -type f))
          else
          ...
          for file in "${files[@]}"; do
            ...
            # Determine topic based on subfolder - SEO categories
            ...
            # Build JSON payload - assign BOTH parent (1158 = SEO) and child topic, plus tag IDs
            if [ ! -z "$excerpt" ]; then
              json_payload=$(jq -n \
                --arg title "$title" \
      ...
      - name: Summary
        ...
        run: |
          if [ "${{ github.event_name }}" == "workflow_dispatch" ]; then
            echo "SEO sync completed (manual trigger - processed all files)"
          else
            echo "SEO sync completed (push trigger - processed changed files)"
          fi
